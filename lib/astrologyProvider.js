...
function calculateAscendant(time, observer, epsilonDeg, ayanamsa) {
  function eclipLonToHoriz(lonDeg) {
    ...
    if (ra < 0) ra += 2 * PI;
    const raHours = ra * RAD2DEG / 15;
    const decDeg = dec * RAD2DEG;

    const horiz = Astronomy.Horizon(time, observer, raHours, decDeg);
 ...
 *
 * The Mahadasha running at birth is the lord of the Moon's birth Nakshatra.
 * Each of the 27 Nakshatras maps cyclically to one of the nine Dasha lords by
 * its index modulo 9 (index 0 = Ashwini -> Ketu, index 1 = Bharani -> Venus,
 * index 2 = Krittika -> Sun, ...). This is the standard Vimshottari mapping.
 *
 * The first Mahadasha is normally already partially elapsed at birth: the
 * fraction of the birth Nakshatra already traversed by the Moon equals the
 * elapsed fraction of that Mahadasha. The remaining balance is therefore:
 *
 *   balance = Mahadasha years x (1 - fraction traversed)
 *
 * When a birth UTC date is supplied, every period is additionally resolved to
 * exact calendar dates (ISO 8601, UTC) using the 365.25-day/year convention
 * that matches the engine's Julian-date math.
 *
 * @param {number} moonSiderealLon - Moon's sidereal longitude
 * @param {Date|string} [birthUtcDate] - birth moment in UTC, for date resolution
 * @returns {{mahaDasha: {lord: string, balance: number, balanceYears: number, balanceMonths: number, balanceDays: number, nakshatraIndex: number, fractionInNakshatra: number, startDate?: string, endDate?: string}, antardashas: Array}}
 */
function calculateDasha(moonSiderealLon, birthUtcDate) {
  ...
  // Canonical table: 7+20+6+10+7+18+16+19+17 = 120.
  const DASHA_YEARS = {
  ...
  // Find which Nakshatra the Moon is in (0-based index; Ashwini = 0).
  const nakIdx = Math.floor(moonSiderealLon / NAKSHATRA_DEGREES) % 27;

  // Fraction of the birth Nakshatra already traversed at birth (0..1).
  const remainder = moonSiderealLon - nakIdx * NAKSHATRA_DEGREES;
  const fractionInNakshatra = remainder / NAKSHATRA_DEGREES;

  // Vimshottari Mahadasha lord = lord of the Moon's birth Nakshatra.
  // Nakshatra lords repeat every 9, so the lord is DASHA_ORDER[idx % 9].
  const lord = DASHA_ORDER[nakIdx % 9];

  // Remaining balance of the current Mahadasha at birth (fractional years).
  ...
  let birthMs = NaN;
  if (birthUtcDate) {
    const parsed = new Date(birthUtcDate).getTime();
    if (isFinite(parsed)) birthMs = parsed;
  }

  const hasDates = isFinite(birthMs);

  // The birth Mahadasha started before birth (elapsed = duration x fraction)
  // and ends balanceYears after birth.
  const mdStartMs = hasDates ? birthMs - dashaDuration * fractionInNakshatra * YEAR_DAYS * DAY_MS : NaN;
  const mdEndMs = hasDates ? birthMs + balanceYears * YEAR_DAYS * DAY_MS : NaN;

  // Build Antardasha (Bhukti) periods of the birth Mahadasha. Each Bhukti is
  // proportional: MD years x Bhukti lord years / 120, sequenced in the
  ...
  for (let i = 0; i < 9; i++) {
    ...
    const fractionOfMd = DASHA_YEARS[bhuktiLord] / TOTAL_YEARS;
    const durationYears = (DASHA_YEARS[lord] * DASHA_YEARS[bhuktiLord]) / TOTAL_YEARS;
    const startMs = hasDates ? mdStartMs + cumFraction * dashaDuration * YEAR_DAYS * DAY_MS : NaN;
    cumFraction += fractionOfMd;
    const endMs = hasDates ? mdStartMs + cumFraction * dashaDuration * YEAR_DAYS * DAY_MS : NaN;
    antardashas.push({
      mahadashaLord: lord,
      ...
      isCurrent: hasDates
        ...
        : fractionInNakshatra >= (cumFraction - fractionOfMd) && fractionInNakshatra < cumFraction,
      startDate: hasDates ? new Date(startMs).toISOString() : null,
      endDate: hasDates ? new Date(endMs).toISOString() : null
    });
  }
  ...
  return {
    mahaDasha: {
      ...
      fractionInNakshatra: fractionInNakshatra,
      startDate: hasDates ? new Date(mdStartMs).toISOString() : null,
      endDate: hasDates ? new Date(mdEndMs).toISOString() : null
    },
...
function calculateChart(dob, birthTime, birthplace, tradition) {
  ...
  const dasha = calculateDasha(moonSiderealLon, utcDate);
